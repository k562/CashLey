...
import com.cashley.app.model.RefferHistoryData
import com.squareup.picasso.Picasso

class HistoryRecyclerAdapter(var list: ArrayList<HistoryData.Data.ReferHistory>, var context: Context) :
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        holder.title.text=list[position].referName
        holder.date.text=list[position].date
        holder.amount.text=list[position].amount
        Picasso.get().load(list[position].referImageUrl).placeholder(R.drawable.baseline_image_24).error(R.drawable.baseline_image_24).into(holder.offerImage)
    }
